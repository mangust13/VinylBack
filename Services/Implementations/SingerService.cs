...
public class SingerService : ISingerService
{
    ...
    public async Task<IEnumerable<SingerDto>> GetSingersByGenresAndStyles(List<int>? genreIds, List<int>? styleIds)
    {
        ...
        if (genreIds != null && genreIds.Any())
            query = query.Where(s => s.Albums.Any(a => a.GenreId.HasValue && genreIds.Contains(a.GenreId.Value)));

        if (styleIds != null && styleIds.Any())
            query = query.Where(s => s.Albums.Any(a => a.StyleId.HasValue && styleIds.Contains(a.StyleId.Value)));
